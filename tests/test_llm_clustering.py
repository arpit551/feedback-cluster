...
def test_llm_fallback_when_cluster_not_found(mock_get_client, client):
    """If LLM says is_new=False but the cluster name doesn't match any existing cluster, create a new one."""
    mock_client = MagicMock()
    mock_get_client.return_value = mock_client
    ...
    idea_id = _add_idea(client, "Some idea that the LLM misclassifies")
    resp = client.post("/cluster/llm", json={"idea_id": idea_id})
    assert resp.status_code == 200
    data = resp.json()
